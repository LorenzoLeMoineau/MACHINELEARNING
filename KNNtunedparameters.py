...
if df is None:
    ...
    if any(col not in df.columns for col in selected_columns) or target_column not in df.columns:
        ...
        fpr, tpr, _ = roc_curve(y_test, best_knn.predict_proba(X_test)[:, 1], pos_label=1)
        roc_auc = auc(fpr, tpr)
        ...
        for i in range(len(selected_columns) - 1):
            for j in range(i + 1, len(selected_columns)):
                ...
                if accuracy >= accuracy_threshold:
                    ...
                    valid_axes_2d.append((visual_features, X, y, best_knn, accuracy, precision, recall, f1, roc_auc))
